...
#### --------Optimize acquisition functions using CMA-ES with Botorch ------------------
...
gp = SingleTaskGP(train_data['input_ids'], model_forward_api.eval(train_data['input_ids'])) 
...
with torch.no_grad():
    while not es.stop():
        solutions = es.ask()  
        solutions = torch.tensor(solutions, device=device, dtype=solutions[0].dtype)

        Y = -UCB(solutions.unsqueeze(-2)) 
        y = Y.view(-1).double().numpy()  

        es.tell(solutions, y)  # return the result to the optimizer
